...
def get_dataset(dataset_dict, path_list, tokenizer, total_samples=500,
    ...
    if dataset_name == 'part_data':
        ...
        if type_dataset == "test":
            test_dataset = dataset["test"].select(range(total_samples))
            test_dataset = test_dataset.map(preprocess_data, batched=True, num_proc=32)
            return test_dataset

        if type_training == TypeTraining.TEACHER:
            train_dataset = train_dataset.map(preprocess_data, batched=True, num_proc=32)
